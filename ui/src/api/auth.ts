...
async function authPost(path: string, body: Record<string, unknown>) {
  const res = await fetch(withPublicBasePath(`/api/auth${path}`), {
    method: "POST",
    credentials: "include",
...
export const authApi = {
  getSession: async (): Promise<AuthSession | null> => {
    const res = await fetch(withPublicBasePath("/api/auth/get-session"), {
      credentials: "include",
  ...
  getProfile: async (): Promise<CurrentUserProfile> => {
    const res = await fetch(withPublicBasePath("/api/auth/profile"), {
      credentials: "include",
      headers: { Accept: "application/json" },
    });
    const payload = await res.json().catch(() => null);
